...
El primer paso para poder estimar los tonos es cargar el audio de la canción en una estructura de datos adecuada, específicamente un vector de muestras.
...
Algoritmos clásicos como pYIN \cite{pYIN} son rápidos. Sin embargo, los enfoques utilizando redes neuronales, como CREPE \cite{CREPE} y SPICE \cite{SPICE}, son el actual estado del arte y gozan de mayor precisión como se puede observar en la figura \ref{fig:crepecomparison}.
...
Teniendo esto en cuenta, que CREPE ha recibido mejoras después de su publicación original \footnote{\url{https://pypi.org/project/crepe/}} y que su integración en el proyecto es más sencilla se considera mejor opción que SPICE y es la elegida para estimar las frecuencias fundamentales.

Soportar el uso de pYIN podría ser útil en un hito posterior, para ofrecer una alternativa más rápida y de menor tamaño, aunque CREPE ofrece la opción de usar modelos reducidos para una inferencia más rápida.


\subsubsection{Funcionamiento de la aplicación}

Primeramente, se analizan los argumentos pasados a la aplicación, definidos con la utilidad \texttt{argparse} de la librería estándar de Python.
...
Una vez analizados los argumentos, se carga el proyecto de canción del fichero de entrada con la librería construida para tal propósito \ref{sec:txtparser}.

Después, se carga el audio de la canción como un vector de muestras normalizado. A partir del audio, se predice una lista de frecuencias fundamentales en hercios empleando CREPE, cada predicción con un nivel de confianza asociado.
...
Para ello primero necesitamos saber la duración de una pulsación, teniendo en cuenta que las pulsaciones por minuto \texttt{"bpm"} contenidas en los metadatos de la canción son multiplicadas por 4 para tener un posicionamiento de notas más preciso:

\begin{displaymath}
	l = 60000 \frac{milisegundos}{minuto} \div (4bpm)\frac{pulsaciones}{minuto} = 15000 / bpm\frac{milisegundos}{pulsacion}
\end{displaymath}

Una vez tenemos esta información, siendo \texttt{"gap"} el retraso respecto al inicio de la canción en milisegundos, \texttt{"x\_1"} el número de pulsaciones en el que empieza la nota, \texttt{"x\_2"} el número de pulsaciones que dura la nota, \texttt{"y\_1"} el milisegundo en el que empieza la nota y \texttt{"y\_2"} el milisegundo en el que acaba la nota:
...
Se calculan los tonos MIDI que corresponden a cada nota, haciendo la media de las frecuencias que estén dentro del principio y fin de cada nota, descartando aquellas que no lleguen al nivel mínimo de confianza establecido en los argumentos y transformando la frecuencia media obtenida de hercios a notas MIDI con la función \texttt{hz\_to\_midi} de librosa.

Finalmente, el programa escribe un archivo de texto con los tonos actualizados en la localización indicada por los argumentos.

\subsection{Tests}

El código añadido durante el desarrollo de este hito está cubierto tanto por tests unitarios como tests de integración al 100\%.

Los tests se han definido con la intención de cubrir un único caso lógico por cada test, lo que permite saber con precisión que funciona exactamente y que no, y cubrir la mayor cantidad posible de casos lógicos.

Para mejorar la legibilidad, reusabilidad y el mantenimiento de los tests los datos de prueba han sido definidos como constantes, en un archivo común si son usados en múltiples módulos o en el módulo correspondiente si solo son usados por uno de ellos.
